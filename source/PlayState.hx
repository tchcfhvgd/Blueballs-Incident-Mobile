...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(SONG.stage)
		{
			...
			case 'incidenti2':
			{
				...
				curStage = 'bundi';
				defaultCamZoom = 1.4;
				bgt.loadGraphic(Paths.image('incidenti2/BB1'));
				bgt.active = false;
				
				add(bgt);

				back1.loadGraphic(Paths.image('incidenti2/BB3'));
				back1.active = false;
				back1.alpha = 0;
				add(back1);

				bgo.loadGraphic(Paths.image('incidenti2/BB6'));
				bgo.active = false;
				bgo.alpha = 0;
				add(bgo);
				bt2.frames = Paths.getSparrowAtlas('incidenti2/bruh');
				bt2.animation.addByPrefix('Idle', 'Idle', 6);
				bt2.animation.play('Idle');
				bt2.scale.set(7, 7);
				bt2.alpha = 0;
				...
				bt3.frames = Paths.getSparrowAtlas('incidenti2/bruh2');
				bt3.animation.addByPrefix('Idle', 'Idle', 6);
				bt3.animation.play('Idle');
				bt3.scale.set(7, 7);
				bt3.alpha = 0;
				add(bt3);
				bbg2.frames = Paths.getSparrowAtlas('incidenti2/sheet2');
				bbg2.animation.addByPrefix('Idle', 'Idle', 24);
				bbg2.animation.play('Idle');	
				bbg2.alpha = 0;
				add(bbg2);

				bt4.frames = Paths.getSparrowAtlas('incidenti2/bapi2');
				bt4.animation.addByPrefix('sus', 'sus', 6);
				bt4.animation.play('sus');
				bt4.scale.set(7, 7);
				bt4.alpha = 0;
				add(bt4);
				bt5.frames = Paths.getSparrowAtlas('incidenti2/bapi');
				bt5.animation.addByPrefix('sus', 'sus', 6);
				bt5.animation.play('sus');
				bt5.scale.set(7, 7);
				bt5.alpha = 0;
				...
				bbg.frames = Paths.getSparrowAtlas('incidenti2/sheet');
				bbg.animation.addByPrefix('Idle', 'Idle', 24);
				bbg.animation.play('Idle');
				bbg.alpha = 0;
				add(bbg);
				...
				bt.frames = Paths.getSparrowAtlas('incidenti2/GlitchSheet');
				bt.animation.addByPrefix('Idle', 'Idle', 24);
				bt.animation.play('Idle');
				bt.scale.set(10, 10);
				bt.alpha = 0;
				...
				sur.loadGraphic(Paths.image('incidenti2/foreground'));
				sur.active = false;
				
				sur.alpha = 0.4;
				sura.loadGraphic(Paths.image('incidenti2/aaa'));
				sura.alpha = 1;

				glitchs.frames = Paths.getSparrowAtlas('incidenti2/glitchAnim');
				glitchs.animation.addByPrefix('g', 'g', 24);
				glitchs.animation.play('g');
				glitchs.scale.set(2,2);
				glitchs.alpha = 0;
				...
				sura2.frames = Paths.getSparrowAtlas('incidenti2/sheeto3');
				sura2.animation.addByPrefix('n', 'n', 24);
				sura2.animation.play('n');
				sura2.scale.set(4, 4);
				sura2.alpha = 0; 
		...
		if (SONG.song.toLowerCase() == 'ommetaphobia')
			{ 
				bluefriend = new Boyfriend(770, 450, 'bfblue'); 
				bfG = new Boyfriend(770, 450, 'bfG');
			}		
		...
		kadeEngineWatermark = new FlxText("@BunnyMalicious @_ezhaltd @FizzyBott");
	...
	override public function update(elapsed:Float)
	{
		...
		if (healthBar.percent < 35)
			iconP1.animation.curAnim.curFrame = 1; //lose icon
		else
	...
	*/
	...
	override function beatHit()
	{
		...
		if (camZooming && FlxG.camera.zoom < 1.35 && curBeat % 8 == 0) // camera zooms
		{
			FlxG.camera.zoom += 0.015;
			camHUD.zoom += 0.03;
		}
